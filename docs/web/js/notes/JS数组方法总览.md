...
> 从索引为0开始，检查数组是否包含value，有则返回匹配到的第一个索引，没有返回-1
>
> 不改变原数组
...
### 16、`lastIndexOf(value)`

> 从最后的索引开始，检查数组是否包含value，有则返回匹配到的第一个索引，没有返回-1
>
> 不改变原数组
...
> 返回数组中第一个匹配fn函数中条件的**值**没有则返回undefined(不改变原数组)
>
> 返回符合条件的**值**
...
> 返回数组中第一个匹配fn函数中条件的**索引**没有则返回undefined(不改变原数组)
>
> 返回符合条件的**索引**
...
let dateEnd = Date.now()
console.log(dateEnd - dateStart)  // 171ms 176ms 188ms 全部不满	足的情况下效率与forEach相当
```
...
### 30、`copyWithin(target, start, end)`
